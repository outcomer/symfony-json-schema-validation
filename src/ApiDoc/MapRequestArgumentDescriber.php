...
final class MapRequestArgumentDescriber implements RouteArgumentDescriberInterface
{
    ...
    public function describe(ArgumentMetadata $argumentMetadata, OA\Operation $operation): void
    {
        ...
        if (isset($resolvedSchema['properties']['headers']['properties'])) {
            Arrays::sortArrayByKeys($resolvedSchema['properties']['headers']['properties']);
        }
        ...
        // Add parameters from path, query, and headers - MapRequest has priority
        if (isset($resolvedSchema['properties']['path'])) {
            $this->addRequestPath(operation: $operation, pathSchema: $resolvedSchema['properties']['path']);
        }

        if (isset($resolvedSchema['properties']['query'])) {
            $this->addRequestQuery(operation: $operation, querySchema: $resolvedSchema['properties']['query']);
        }

        if (isset($resolvedSchema['properties']['headers'])) {
            $this->addRequestHeaders(operation: $operation, headerSchema: $resolvedSchema['properties']['headers']);
        }
    }

    /**
     * Adds path parameters to OpenAPI operation from schema
     */
    private function addRequestPath(OA\Operation $operation, array $pathSchema): void
    {
        $this->clearExistingParameters($operation, 'path');

        foreach ($pathSchema['properties'] as $name => $property) {
            $this->addParameter(
                operation: $operation,
                name: $name,
                in: 'path',
                property: $property,
                required: in_array(needle: $name, haystack: $pathSchema['required'] ?? [])
            );
        }
    }

    /**
     * Adds query parameters to OpenAPI operation from schema
     */
    private function addRequestQuery(OA\Operation $operation, array $querySchema): void
    {
        $this->clearExistingParameters($operation, 'query');

        foreach ($querySchema['properties'] as $name => $property) {
            $this->addParameter(
                operation: $operation,
                name: $name,
                in: 'query',
                property: $property,
                required: in_array(needle: $name, haystack: $querySchema['required'] ?? [])
            );
        }
    }

    /**
     * Adds header parameters to OpenAPI operation from schema
     */
    private function addRequestHeaders(OA\Operation $operation, array $headerSchema): void
    {
        $this->clearExistingParameters($operation, 'header');

        if (isset($headerSchema['properties'])) {
            // Initialize parameters array only when header schema exists
            if ($operation->parameters === Generator::UNDEFINED) {
                $operation->parameters = [];
            }

            foreach ($headerSchema['properties'] as $name => $property) {
                $this->addParameter(
                    operation: $operation,
                    name: $name,
                    in: 'header',
                    property: $property,
                    required: in_array(needle: $name, haystack: $headerSchema['required'] ?? [])
                );
            }
        }
    }
    ...
    {
        $this->clearExistingRequestBody($operation);
    ...
    /**
     * Clears existing parameters of specified type (MapRequest has priority)
     */
    private function clearExistingParameters(OA\Operation $operation, string $in): void
    {
        if ($operation->parameters !== Generator::UNDEFINED && is_array($operation->parameters)) {
            $operation->parameters = array_values(array_filter($operation->parameters, function ($param) use ($in) {
                return !($param instanceof OA\Parameter && $param->in === $in);
            }));
        }
    }

    /**
     * Clears existing request body (MapRequest has priority)
     */
    private function clearExistingRequestBody(OA\Operation $operation): void
    {
